    ...
    P::Fp2Params: Fp2Parameters<Fp = F>,
{
    fn is_eq<CS: ConstraintSystem<F>>(&self, mut cs: CS, other: &Self) -> Result<Boolean, SynthesisError> {
        ...
        let b2 = self.c2.is_eq(cs.ns(|| "c2_is_eq"), &other.c2)?;
        let b0_and_b1 = Boolean::and(cs.ns(|| "b0_and_b1"), &b0, &b1)?;

        Boolean::and(cs.ns(|| "b0_and_b1_and_b2"), &b0_and_b1, &b2)
    }
}
...
impl<P, F: PrimeField> ToBytesLEGadget<F> for Fp6Gadget<P, F>
where
    P: Fp6Parameters,
    P::Fp2Params: Fp2Parameters<Fp = F>,
{
    fn to_bytes_le<CS: ConstraintSystem<F>>(&self, mut cs: CS) -> Result<Vec<UInt8>, SynthesisError> {
        let mut c0 = self.c0.to_bytes_le(cs.ns(|| "c0"))?;
        let mut c1 = self.c1.to_bytes_le(cs.ns(|| "c1"))?;
        let mut c2 = self.c2.to_bytes_le(cs.ns(|| "c2"))?;
...
impl<P, F: PrimeField> ToBytesBEGadget<F> for Fp6Gadget<P, F>
where
    P: Fp6Parameters,
    P::Fp2Params: Fp2Parameters<Fp = F>,
{
    fn to_bytes_be<CS: ConstraintSystem<F>>(&self, mut cs: CS) -> Result<Vec<UInt8>, SynthesisError> {
        ...
        let mut c2 = self.c2.to_bytes_be(cs.ns(|| "c2"))?;
...
impl<P, F: PrimeField> ToConstraintFieldGadget<F> for Fp6Gadget<P, F>
where
    P: Fp6Parameters,
    P::Fp2Params: Fp2Parameters<Fp = F>,
{
    fn to_constraint_field<CS: ConstraintSystem<F>>(&self, mut cs: CS) -> Result<Vec<FpGadget<F>>, SynthesisError> {
        ...
        res.extend_from_slice(&self.c0.to_constraint_field(cs.ns(|| "fp6_c0_to_constraint_field"))?);
        res.extend_from_slice(&self.c1.to_constraint_field(cs.ns(|| "fp6_c1_to_constraint_field"))?);
        res.extend_from_slice(&self.c2.to_constraint_field(cs.ns(|| "fp6_c2_to_constraint_field"))?);
