...
impl<P: Bls12Parameters> ToBytesBEGadget<P::Fp> for G2PreparedGadget<P> {
    #[inline]
    fn to_bytes_be<CS: ConstraintSystem<P::Fp>>(&self, mut cs: CS) -> Result<Vec<UInt8>, SynthesisError> {
        let mut bytes = Vec::new();
        for (i, coeffs) in self.ell_coeffs.iter().enumerate() {
            let mut cs = cs.ns(|| format!("Iteration {}", i));
            bytes.extend_from_slice(&coeffs.0.to_bytes_be(&mut cs.ns(|| "c0"))?);
            bytes.extend_from_slice(&coeffs.1.to_bytes_be(&mut cs.ns(|| "c1"))?);
        }
        Ok(bytes)
    }
