...
pub struct And<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for And<M> {
    type Memory = M;

    /// Returns the mnemonic for the `And` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for And<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for And<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for And<M> {
...
impl<M: Memory> Into<Instruction<M>> for And<M> {
