...
use snarkvm_circuits::{Literal, Parser, ParserResult, ShrWrapped as CircuitShrWrapped};
...
pub struct ShrWrapped<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for ShrWrapped<M> {
    type Memory = M;

    /// Returns the mnemonic for the `ShrWrapped` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "shr.w"
    }

    /// Parses a string into an `ShrWrapped` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for ShrWrapped<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for ShrWrapped<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for ShrWrapped<M> {
...
impl<M: Memory> Into<Instruction<M>> for ShrWrapped<M> {
