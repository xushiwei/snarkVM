...
use snarkvm_circuits::{Literal, Parser, ParserResult, ShlChecked as CircuitShlChecked};
...
pub struct ShlChecked<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for ShlChecked<M> {
    type Memory = M;

    /// Returns the mnemonic for the `ShlChecked` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "shl.c"
    }

    /// Parses a string into an `ShlChecked` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for ShlChecked<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for ShlChecked<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for ShlChecked<M> {
...
impl<M: Memory> Into<Instruction<M>> for ShlChecked<M> {
    /// Converts the operation into an instruction.
    fn into(self) -> Instruction<M> {
        Instruction::ShlChecked(self)
    }
}
