...
pub struct Or<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for Or<M> {
    type Memory = M;

    /// Returns the mnemonic for the `Or` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for Or<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for Or<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for Or<M> {
...
impl<M: Memory> Into<Instruction<M>> for Or<M> {
