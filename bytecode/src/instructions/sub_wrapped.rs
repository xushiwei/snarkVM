...
use snarkvm_circuits::{Literal, Parser, ParserResult, SubWrapped as CircuitSubWrapped};
...
pub struct SubWrapped<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for SubWrapped<M> {
    type Memory = M;

    /// Returns the mnemonic for the `SubWrapped` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for SubWrapped<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for SubWrapped<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for SubWrapped<M> {
...
impl<M: Memory> Into<Instruction<M>> for SubWrapped<M> {
