...
use snarkvm_circuits::{DivWrapped as CircuitDivWrapped, Literal, Parser, ParserResult};
...
pub struct DivWrapped<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for DivWrapped<M> {
    type Memory = M;

    /// Returns the mnemonic for the `DivWrapped` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "div.w"
    }

    /// Parses a string into an `DivWrapped` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for DivWrapped<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for DivWrapped<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for DivWrapped<M> {
...
impl<M: Memory> Into<Instruction<M>> for DivWrapped<M> {
