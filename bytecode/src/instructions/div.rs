...
pub struct Div<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for Div<M> {
    type Memory = M;
...
impl<M: Memory> fmt::Display for Div<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for Div<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for Div<M> {
...
impl<M: Memory> Into<Instruction<M>> for Div<M> {
...
mod tests {
    ...
    fn test_div_field() {
        ...
        Div::<Stack<Circuit>>::from_str("r2 r0 r1", &memory).evaluate(&memory);
