...
use snarkvm_circuits::{Literal, Nand as CircuitNand, Parser, ParserResult};
...
pub struct Nand<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for Nand<M> {
    type Memory = M;

    /// Returns the mnemonic for the `Nand` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for Nand<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for Nand<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for Nand<M> {
...
impl<M: Memory> Into<Instruction<M>> for Nand<M> {
