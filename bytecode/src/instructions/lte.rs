...
pub struct LessThanOrEqual<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for LessThanOrEqual<M> {
    type Memory = M;
...
impl<M: Memory> fmt::Display for LessThanOrEqual<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for LessThanOrEqual<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for LessThanOrEqual<M> {
...
impl<M: Memory> Into<Instruction<M>> for LessThanOrEqual<M> {
...
mod tests {
    ...
    fn test_less_than_or_equal_field() {
        ...
        LessThanOrEqual::<Stack<Circuit>>::from_str("r2 r0 r1", &memory).evaluate(&memory);
        assert_eq!(expected, memory.load(&Register::new(2)));
    }

    #[test]
    fn test_less_than_or_equal_scalar() {
