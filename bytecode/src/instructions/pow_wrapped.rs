...
use snarkvm_circuits::{Literal, Parser, ParserResult, PowWrapped as CircuitPowWrapped};
...
pub struct PowWrapped<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for PowWrapped<M> {
    type Memory = M;

    /// Returns the mnemonic for the `PowWrapped` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "pow.w"
    }

    /// Parses a string into an `PowWrapped` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for PowWrapped<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for PowWrapped<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for PowWrapped<M> {
...
impl<M: Memory> Into<Instruction<M>> for PowWrapped<M> {
