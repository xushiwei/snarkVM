...
use snarkvm_circuits::{AddChecked as CircuitAddChecked, Literal, Parser, ParserResult};
...
/// AddCheckeds `first` with `second`, checks for overflow, and stores the result in `destination`.
pub struct AddChecked<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for AddChecked<M> {
    type Memory = M;

    /// Returns the mnemonic for the `AddChecked` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "add.c"
    }

    /// Parses a string into an `AddChecked` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for AddChecked<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for AddChecked<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for AddChecked<M> {
...
impl<M: Memory> Into<Instruction<M>> for AddChecked<M> {
    /// Converts the operation into an instruction.
    fn into(self) -> Instruction<M> {
        Instruction::AddChecked(self)
    }
}
