...
use snarkvm_circuits::{prelude::Inv as CircuitInv, Literal, Parser, ParserResult};
...
pub struct Inv<M: Memory> {
    operation: UnaryOperation<M::Environment>,
}

impl<M: Memory> Operation for Inv<M> {
    type Memory = M;
...
impl<M: Memory> fmt::Display for Inv<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for Inv<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: UnaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for Inv<M> {
...
impl<M: Memory> Into<Instruction<M>> for Inv<M> {
...
mod tests {
    ...
    fn test_inv_field() {
        ...
        Inv::<Stack<Circuit>>::from_str("r1 r0", &memory).evaluate(&memory);
