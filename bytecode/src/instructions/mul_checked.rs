...
use snarkvm_circuits::{Literal, MulChecked as CircuitMulChecked, Parser, ParserResult};
...
pub struct MulChecked<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for MulChecked<M> {
    type Memory = M;

    /// Returns the mnemonic for the `MulChecked` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "mul.c"
    }

    /// Parses a string into an `MulChecked` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for MulChecked<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for MulChecked<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for MulChecked<M> {
...
impl<M: Memory> Into<Instruction<M>> for MulChecked<M> {
    /// Converts the operation into an instruction.
    fn into(self) -> Instruction<M> {
        Instruction::MulChecked(self)
    }
}
