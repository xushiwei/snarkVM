...
use snarkvm_circuits::{Literal, Parser, ParserResult, ShrChecked as CircuitShrChecked};
...
pub struct ShrChecked<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for ShrChecked<M> {
    type Memory = M;

    /// Returns the mnemonic for the `ShrChecked` operation.
    #[inline]
    fn mnemonic() -> &'static str {
        "shr.c"
    }

    /// Parses a string into an `ShrChecked` operation.
    #[inline]
...
impl<M: Memory> fmt::Display for ShrChecked<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for ShrChecked<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for ShrChecked<M> {
...
impl<M: Memory> Into<Instruction<M>> for ShrChecked<M> {
    /// Converts the operation into an instruction.
    fn into(self) -> Instruction<M> {
        Instruction::ShrChecked(self)
    }
}
