...
pub struct GreaterThanOrEqual<M: Memory> {
    operation: BinaryOperation<M::Environment>,
}

impl<M: Memory> Operation for GreaterThanOrEqual<M> {
    type Memory = M;
...
impl<M: Memory> fmt::Display for GreaterThanOrEqual<M> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.operation)
    }
}

impl<M: Memory> FromBytes for GreaterThanOrEqual<M> {
    fn read_le<R: Read>(mut reader: R) -> IoResult<Self> {
        Ok(Self { operation: BinaryOperation::read_le(&mut reader)? })
    }
}

impl<M: Memory> ToBytes for GreaterThanOrEqual<M> {
...
impl<M: Memory> Into<Instruction<M>> for GreaterThanOrEqual<M> {
...
mod tests {
    ...
    #[test]
    fn test_greater_than_or_equal_field() {
        ...
        GreaterThanOrEqual::<Stack<Circuit>>::from_str("r2 r0 r1", &memory).evaluate(&memory);
        assert_eq!(expected, memory.load(&Register::new(2)));
    }

    #[test]
    fn test_greater_than_or_equal_scalar() {
